...
const remove=async(req,res,next)=>{
    try{
        const {name}=req.body;
        const {id}=req.user;
        const match_room= await room_Model.findOne({name:name});
        if(!match_room)return res.status(404).json({message:"room not found"});
        if(!match_room.owner.equals(id))return res.status(404).json({message:"you are not authorised to delete the room"});
        const x=await room_Model.deleteOne({_id:match_room._id});
